...
export async function uploadImageToSupabase(file: File): Promise<string> {
  if (!supabase) {
    throw new Error(
      "Upload is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to your .env."
    );
  }
  ...
  if (error) {
    const msg =
      error.message === "Bucket not found"
        ? `Bucket "${LOGOS_BUCKET}" not found. In Supabase Dashboard → Storage, check the bucket name matches exactly, or set VITE_SUPABASE_LOGOS_BUCKET in .env to your bucket name.`
